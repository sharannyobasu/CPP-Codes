...
bool check_equal(node*root1, node*root2)
{
    ...
    if(root1 and root2)
    {
        return (root1->data==root2->data and check_equal(root1->left, root2->left) and check_equal(root1->right, root2->right));
    }
}
...
int main()
{
    ...
    printree(root2);
    cout<<endl;
    if(check_equal(root1, root2))
    {
        cout<<"Trees are equal :)";
    }
    else
    {
        cout<<"Trees aren't equal :(";
    }
}